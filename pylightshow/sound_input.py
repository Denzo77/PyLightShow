...
window = signal.hamming(BLOCKSIZE, False)  # Hamming window for the FFT to give us a sane plot.
...
def audio_callback(indata, frames, time, status):
    ...
    if any(indata):  # not sure if this is needed.
        temp = np.abs(fftpack.rfft((indata[:, 0] * window), overwrite_x=True))  # window data, get FFT find absolute.
